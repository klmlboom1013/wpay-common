...
@Log4j2
@Aspect
@Component(value = "BaseExternalAdapterAspect")
public class ExternalAdapterAspect extends BaseAspect {
    @Before("execution(* com.wpay..adapter.out.external.*External.*Run(..))")
    @Override
    ...
    @After("execution(* com.wpay..adapter.out.external.*External.*Run(..))")
    @Override
    public void after(JoinPoint joinPoint) {
        log.debug("[After] => {}", joinPoint.getSignature().getName());
    }

    @AfterReturning(pointcut = "execution(* com.wpay..adapter.out.external.*External.*Run(..))", returning = "result")
    @Override
    ...
    @AfterThrowing(pointcut = "execution(* com.wpay..adapter.out.external.*External.*Run(..))", throwing = "e")
    @Override
    public void afterThrowing(JoinPoint joinPoint, Throwable e) {
        log.error("[AfterThrowing] => {} [{}] => {}", joinPoint.getSignature().getName(), e.getClass().getName(), e.getMessage());
        super.afterThrowingForExternal(joinPoint,e);
    }
}
