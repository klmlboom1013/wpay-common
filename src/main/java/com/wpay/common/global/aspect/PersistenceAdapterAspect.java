...
@Log4j2
@Aspect
@Component(value = "basePersistenceAdapterAspect")
public class PersistenceAdapterAspect extends BaseAspect {

    @Before("execution(* com.wpay..adapter.out.persistence.*Persistence.*Run(..))")
    @Override
    ...
    @AfterReturning(pointcut = "execution(* com.wpay..adapter.out.persistence.*Persistence.*Run(..))", returning = "result")
    @Override
    ...
    @AfterThrowing(pointcut = "execution(* com.wpay..adapter.out.persistence.*Persistence.*Run(..))", throwing = "e")
    @Override
